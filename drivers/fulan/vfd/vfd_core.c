/*$Source: D:/STB_BASE_HAL/vfd.fulan.7167/vfd_core.c,v $*/
/*****************************文件头部注释*************************************/
//
//			Copyright (C), 2008-2013, AV Frontier Tech. Co., Ltd.
//
//
// 文 件 名：	$RCSfile: vfd_core.c,v $
//
// 创 建 者：	D02SH
//
// 创建时间：	2008.11.19
//
// 最后更新：	$Date: 2011/08/03 02:45:29 $
//
//				$Author: d27lwj $
//
//				$Revision: 1.2 $
...
//add end
	...
	switch(data->dataType)
	{
		...
		case YWPANEL_DATATYPE_SETVFDSTATE:
		{
		   I2CData->writeBuff[2] = data->data.VfdState.state;
		}
		break;
...
YWPANEL_VFDSTATE_t YWPANEL_FP_GetVFDStatus(void)
{
	...
	if((data.data.VfdState.state < YWPANEL_VFDSTATE_STANDBYOFF) ||(data.data.VfdState.state > YWPANEL_VFDSTATE_STANDBYON) )
	{
		return YWPANEL_VFDSTATE_UNKNOW;
	}

	return data.data.VfdState.state;
}
...
bool  YWPANEL_FP_SetVFDStatus(YWPANEL_VFDSTATE_t state)
{
	...
	data.data.VfdState.state = state;
...
/*************************************************************
	函数名称:	YWPANEL_VFD_ShowContentOff
	函数说明:	关显示
	算法说明:	无
	调用说明:	Calls							Called By
			   ------						  ------------
...
/*****************************************************************************/
//	函数名称:	YWPANEL_VFD_ShowTimeOff
//	函数说明:	Set ShowTimeoff
//	输入参数:	NA
//	输出参数:	NA
//	返 回 值:	NA
//	修改记录:	日		 期 	 作 	 者 	  修 定
...
/*****************************************************************************/
//	函数名称:	YWPANEL_VFD_SetBrightness
//	函数说明:	Set VFD brightness
//	输入参数:	NA
//	输出参数:	NA
//	返 回 值:	NA
//	修改记录:	日		 期 	 作 	 者 	  修 定
...
/*****************************************************************************/
//	函数名称:	YWPANEL_VFD_DETECT
//	函数说明:	Detect what kind of vfd using
//	输入参数:	NA
//	输出参数:	NA
//	返 回 值:	NA
//	修改记录:	日		 期 	 作 	 者 	  修 定
...
int YWPANEL_VFD_DETECT(void)
{
	...
	YWVFD_INFO.vfd_type = YWVFD_UNKNOW; //初始化为未知
	//printk("%s:%d\n", __FUNCTION__, __LINE__);
...
/*************************************************************
	函数名称:	YWPANEL_VFD_Init
	函数说明:	面板初始化 和YW_VFD_TERM相对
	算法说明:	无
	调用说明:	Calls							Called By
			   ------						  ------------
				AAAAA() 					   BBBBB()
...
#if 0  //I don't need this lwj
EXPORT_SYMBOL(YWPANEL_VFD_Init);
EXPORT_SYMBOL(YWPANEL_VFD_Term);
EXPORT_SYMBOL(YWPANEL_VFD_GetRevision);
EXPORT_SYMBOL(YWPANEL_VFD_ShowIco);
EXPORT_SYMBOL(YWPANEL_VFD_ShowString);
EXPORT_SYMBOL(YWPANEL_VFD_ShowTime);
EXPORT_SYMBOL(YWPANEL_VFD_ShowTimeOff);
EXPORT_SYMBOL(YWPANEL_VFD_SetBrightness);
EXPORT_SYMBOL(YWPANEL_VFD_SetLed);
EXPORT_SYMBOL(YWPANEL_FP_GetVFDStatus);
EXPORT_SYMBOL(YWPANEL_FP_SetVFDStatus);
EXPORT_SYMBOL(YWPANEL_FP_GetCpuStatus);
EXPORT_SYMBOL(YWPANEL_FP_SetCpuStatus);
EXPORT_SYMBOL(YWPANEL_FP_ControlTimer);
EXPORT_SYMBOL(YWPANEL_FP_GetPowerOnStatus);
EXPORT_SYMBOL(YWPANEL_FP_SetPowerOnStatus);
EXPORT_SYMBOL(YWPANEL_FP_GetTime);
EXPORT_SYMBOL(YWPANEL_FP_SetTime);
EXPORT_SYMBOL(YWPANEL_FP_GetStandByKey);
EXPORT_SYMBOL(YWPANEL_FP_SetStandByKey);
EXPORT_SYMBOL(YWPANEL_FP_GetBlueKey);
EXPORT_SYMBOL(YWPANEL_FP_SetBlueKey);
EXPORT_SYMBOL(YWPANEL_LBD_SetStatus);
EXPORT_SYMBOL(YWPANEL_FP_GetStartUpState);
EXPORT_SYMBOL(YWPANEL_FP_SetPowerOnTime);
EXPORT_SYMBOL(YWPANEL_FP_GetPowerOnTime);
EXPORT_SYMBOL(YWPANEL_VFD_GetKeyValue);
#endif
/*int	vfd_module_init(void)
{
	printk("vfd module initialized... \n");
	return 0;
}
void vfd_module_exit(void)
{
}
