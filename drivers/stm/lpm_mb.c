...
#warning "when elf parser will be available in kernel, this code will be removed"

/* Check elf file identity */
static unsigned int ELF64_checkIdent(Elf64_Ehdr *hdr)
{
	return memcmp(hdr->e_ident, ELFMAG, SELFMAG);
}

static inline int ELF64_valid_offset(struct ELF64info *elfinfo, Elf64_Off off,
	Elf64_Off struct_size)
{
	return off + struct_size <= elfinfo->size;
}

/* Initialise in-memory ELF file */
static struct ELF64info *ELF64_initFromMem(uint8_t *elffile,
				uint32_t size, int mmapped)
{
	Elf64_Shdr	*sec;
	struct ELF64info *elfinfo;
	int i;

	elfinfo = (struct ELF64info *)kmalloc(sizeof(struct ELF64info), GFP_KERNEL);
	...
	elfinfo->header = (Elf64_Ehdr *)elffile;
	...
	if (ELF64_checkIdent((Elf64_Ehdr *)elffile))
		goto fail;

	/* Make sure it is 64 bit, little endian and current version */
	if (elffile[EI_CLASS] != ELFCLASS64 ||
	...
	if ((elfinfo->header->e_ehsize != sizeof(Elf64_Ehdr)) ||
	    (elfinfo->header->e_phentsize != sizeof(Elf64_Phdr)) ||
	    (elfinfo->header->e_shentsize != sizeof(Elf64_Shdr)))
		goto fail;
	...
	if (!ELF64_valid_offset(elfinfo, elfinfo->header->e_shoff,
			      sizeof(Elf64_Shdr) * elfinfo->numsections) ||
	    !ELF64_valid_offset(elfinfo, elfinfo->header->e_phoff,
			      sizeof(Elf64_Phdr) * elfinfo->numpheaders))
		goto fail;

	/* Cache commonly-used addresses and values */
	elfinfo->secbase = (Elf64_Shdr *)(elfinfo->base +
				(elfinfo->header)->e_shoff);
	elfinfo->progbase = (Elf64_Phdr *)(elfinfo->base +
	...
	for (i = 0; i < elfinfo->numsections; i++) {
		...
		if (!ELF64_valid_offset(elfinfo, shdr->sh_offset,
				      shdr->sh_size))
			goto fail;
	}
	...
	for (i = 0; i < elfinfo->numpheaders; i++) {
		...
		if (!ELF64_valid_offset(elfinfo, phdr->p_offset,
				      phdr->p_filesz))
			goto fail;
...
int stm_lpm_setup_ir(int num_of_sequence, int byte_count,int protocol, struct stm_lpm_ir_setting *byte_sequence){
        ...
        if(num_of_sequence*byte_count < MAX_SEQ_IN_MAILBOX){
                ...
                msg[2]=byte_count;
                ir_msg_count=3;
		memcpy(&msg[ir_msg_count], &byte_sequence, (num_of_sequence*4));
                ...
                LPM_FILL_MSG(send_msg,STM_LPM_MSG_LGWR_OFFSET,msg,1,MSG_ID_AUTO,SBC_REPLY_YES);
                err=lpm_exchange_msg(&send_msg,&response);

                if(response.command_id != STM_LPM_MSG_ERR){
			memcpy((void*)&offset, &response.msg_data[2], 4);
                       /* Write large messg in DMEM*/
                        lpm_write8(lpm_drv, SBC_DATA_MEMORY_OFFSET+offset, STM_LPM_MSG_SET_IR);
                        lpm_write8(lpm_drv, SBC_DATA_MEMORY_OFFSET+(offset+1), 0);
                        lpm_write8(lpm_drv, SBC_DATA_MEMORY_OFFSET+(offset+2), (protocol<<4));
                        lpm_write8(lpm_drv, SBC_DATA_MEMORY_OFFSET+(offset+3), num_of_sequence);
                        lpm_write8(lpm_drv, SBC_DATA_MEMORY_OFFSET+(offset+4), byte_count);
                        ir_msg_count=5;
                        memcpy((void *)lpm_drv->lpm_mem_base+SBC_DATA_MEMORY_OFFSET+(offset+ir_msg_count), &byte_sequence, (num_of_sequence*4));

                        /*BKBD_WRITE*/
                        msg[0]=((SIZE_DATA_BYTES(num_of_sequence))&(0xff));
                        msg[1]=(((SIZE_DATA_BYTES(num_of_sequence))&(0xff00))>>8);
                        memcpy(&msg[2],&response.msg_data[2],4);
                        LPM_FILL_MSG(send_msg,STM_LPM_MSG_BKBD_WRITE,msg,6,MSG_ID_AUTO,SBC_REPLY_YES);
                        err=lpm_exchange_msg(&send_msg,&response);
                        if(response.command_id != STM_LPM_MSG_ERR){
                        }
                        else{
...
datasize, int *data)
{
	...
	if(response.command_id == STM_LPM_MSG_BKBD_READ){
		// 1 large msg
		memcpy((void*)&offset,&response.msg_data[2], 4);
		memcpy((void *)data,(const void*)(SBC_DATA_MEMORY_OFFSET+offset+4),size);
       	memcpy((void *)validsize,(const void *)(SBC_DATA_MEMORY_OFFSET+(offset+2)), 2);
	}
	else{
		//2 Small msg
...
static irqreturn_t lpm_isr(int this_irq, void *params){
    ...
    if((msg_read[0] & STM_LPM_MSG_REPLY) ||
	  ((msg_read[0] & STM_LPM_MSG_BKBD_READ) && (lpm_drv_p->reply_from_sbc==0))){
...
static int lpm_msg_request_task(void *params){
	...
	lpm_drv_p=(struct stm_lpm_driver_data *)params;
		...
		err=wait_event_interruptible(lpm_drv_p->stm_lpm_wait_queue,lpm_drv_p->query_from_sbc == 1);
                if(err>=0 && msg_p->command_id !=0){
                	lpm_debug("recd command id %x \n",msg_p->command_id);
                        	...
                        	/*case STM_LPM_MSG_SET_TRIM:
                                ...
                                case STM_LPM_MSG_SET_FP:
                                        ...
                                        default:
                                                ...
                                                msg[1]=-EINVAL;
                                                LPM_FILL_MSG(send_msg,STM_LPM_MSG_ERR,msg,2,msg_p->transaction_id,SBC_REPLY_NO);
						...
						memcpy(msg, &msg_p->msg_data, 2);
                                                LPM_FILL_MSG(send_msg,STM_LPM_MSG_TRACE_DATA|STM_LPM_MSG_REPLY,msg,2,msg_p->transaction_id,SBC_REPLY_NO);
                                                lpm_exchange_msg(&send_msg,NULL);
                                        break;
                                        case STM_LPM_MSG_VER:
                                                msg[0]= STM_LPM_MAJOR_PROTO_VER << 4 | STM_LPM_MINOR_PROTO_VER;
                                                msg[1]= STM_LPM_MAJOR_SOFT_VER << 4 | STM_LPM_MINOR_SOFT_VER;
                                                msg[2]= STM_LPM_PATCH_SOFT_VER << 4 | STM_LPM_BUILD_MONTH;
                                                msg[3]= STM_LPM_BUILD_DAY;
                                                msg[4]= STM_LPM_BUILD_YEAR;
                                                LPM_FILL_MSG(send_msg,STM_LPM_MSG_VER|STM_LPM_MSG_REPLY,msg,5,msg_p->transaction_id,SBC_REPLY_NO);
                                                lpm_exchange_msg(&send_msg,NULL);
                                        break;
                                        case STM_LPM_MSG_BKBD_READ:
                                                memcpy(&offset, &msg_p->msg_data[2], 4);
												memcpy(&size,&msg_p->msg_data[0],2);
                                                memcpy((void*)&trace_data,(void*)(lpm_drv_p->lpm_mem_base+SBC_DATA_MEMORY_OFFSET+offset),size);
                                                if(trace_data.commant_id==STM_LPM_MSG_TRACE_DATA){
                                                        //Print data here
                                                }
                                                LPM_FILL_MSG(send_msg,STM_LPM_MSG_BKBD_READ|STM_LPM_MSG_REPLY,msg,2,msg_p->transaction_id,SBC_REPLY_NO);
                                                lpm_exchange_msg(&send_msg,NULL);
                                }
                                msg_p->command_id=0;
                        }
...
static int lpm_send_msg(struct stm_lpm_message *msg, unsigned char msg_size){
            /* For ORLY only*/
        int err=0, count;
        int *tmp_i = (int *)msg;

        for (count = (msg_size+1)/4 ; count >= 0 ; count--) {
                lpm_write32(lpm_drv, (MBX_WRITE_STATUS1+(count*4)), *(tmp_i+count));
        }
        return err;
}
...
int lpm_exchange_msg(struct stlpm_internal_send_msg * send_msg, struct stm_lpm_message *response)
{
       ...
       lpm_debug("lpm_exchange_msg \n");
        ...
        lpm_debug("Sending msg {%x, %x ",lpm_msg.command_id,lpm_msg.transaction_id);
        ...
        switch(send_msg->reply_type){
                ...
                case  SBC_REPLY_YES:
                        ...
                        err=wait_event_interruptible_timeout(lpm_drv->stm_lpm_wait_queue, lpm_drv->reply_from_sbc == 1,msecs_to_jiffies(100));
                break;
                case SBC_REPLY_NO:
                        goto exit_fun;
                break;
        ...
        lpm_debug("recd reply error %x {%x, %x ",err, lpm_drv->fw_reply_msg.command_id,
        ...
        lpm_debug("} \n");
        if(err){
                memcpy(response,&lpm_drv->fw_reply_msg,sizeof(struct stm_lpm_message));
                ...
                else if (response->command_id==STM_LPM_MSG_BKBD_READ)
                {
                        ...
                        lpm_debug("Received response tran ID %x instead of %x \n",response->transaction_id,lpm_msg.transaction_id );
                        err=-EAGAIN; /* invalid trans id*/
                }
        }
        else {
...
static int lpm_load_segment(struct stm_lpm_driver_data *lpm_drv_p, struct ELF64info *elfinfo, int i)
{
        ...
        memcpy_toio(((void*)lpm_drv_p->lpm_mem_base + offset), data + phdr->p_offset, size);
	return 0;
}


static int lpm_load_fw(const struct firmware *fw, struct stm_lpm_driver_data *lpm_drv_p)
{
	struct ELF64info *elfinfo = NULL;
        int i;

        if (!fw) {
                lpm_debug("Unable to load LPM firmware: not present?\n");
                return -EINVAL;
        }

        lpm_debug("Found sbc f/w \n");
        elfinfo = (struct ELF64info *)ELF64_initFromMem((uint8_t *)fw->data,
                                                        fw->size, 0);
	if (elfinfo == NULL){
                return -ENOMEM;
	}
...
static int lpm_load_firmware(struct device *dev)
{
    ...
    BUG_ON(result >= sizeof(lpm_drv_p->fw_name)); /* was the string truncated? */

    lpm_debug("LPM: Loading Firmware (%s)...\n", lpm_drv_p->fw_name);

    err = request_firmware_nowait(THIS_MODULE, 1, lpm_drv_p->fw_name,
    dev, (struct stm_lpm_driver_data *)dev->platform_data,
    (void *)lpm_load_fw);
    if (err)
        return -ENOMEM;
...
static int __init stm_lpm_probe(struct platform_device *pdev){
        ...
        if (!(lpm_drv->lpm_mem_base =
                (unsigned int)devm_ioremap_nocache(&pdev->dev, res->start,
        ...
        if(devm_request_irq(&pdev->dev, res->start, lpm_isr,
                                IRQF_DISABLED, "ORLY", (void*)lpm_drv)<0){
	...
	/*Semaphore initialization*/
        ...
        lpm_drv->th = kthread_run(lpm_msg_request_task,(void *)lpm_drv,"lpm_read_task");
...
static int __init stm_lpm_init(void){
        ...
        printk("STM_LPM driver registered err %x\n",err);
        return err;
}
