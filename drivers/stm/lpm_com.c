...
int stm_lpm_get_version(struct stm_lpm_version * driver_version, struct stm_lpm_version * fw_version){
        int err = 0;
...
//        lpm_debug("added to test I2C again and again after probe \n");
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_VER,NULL,MSG_ZERO_SIZE,MSG_ID_AUTO,SBC_REPLY_YES);
...
int stm_lpm_configure_wdt(u16 time_in_ms){
	...
	memcpy(msg, &time_in_ms, 2);
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_SET_WDT,msg,2,MSG_ID_AUTO,SBC_REPLY_YES);
...
int stm_lpm_get_fw_state(enum stm_lpm_sbc_state *fw_state){
        ...
        if(fw_state==NULL)
                return -EINVAL;
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_GET_STATUS,NULL,MSG_ZERO_SIZE,MSG_ID_AUTO,SBC_REPLY_YES);
...
int stm_lpm_reset(enum stm_lpm_reset_type reset_type){
        ...
        if(!reset_type)
                return -EINVAL;
        msg=reset_type;
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_GEN_RESET,&msg,1,MSG_ID_AUTO,SBC_REPLY_NO);
        err=lpm_exchange_msg(&send_msg,&response);
        return err;
}
...
int stm_lpm_set_wakeup_device(unsigned char devices){
        ...
        struct stlpm_internal_send_msg send_msg={0};
        msg[0] = devices;
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_SET_WUD,msg,2,MSG_ID_AUTO,SBC_REPLY_NO_IRQ);
        err = lpm_exchange_msg(&send_msg,&response);
        return err;
}
...
int stm_lpm_set_wakeup_time(unsigned int timeout){
        ...
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_SET_TIMER,msg,4,MSG_ID_AUTO,SBC_REPLY_YES);
        err= lpm_exchange_msg(&send_msg,&response);
        return err;
}
...
int stm_lpm_set_rtc(struct rtc_time new_rtc){
        ...
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_SET_RTC,msg,3,MSG_ID_AUTO,SBC_REPLY_YES);
        err=lpm_exchange_msg(&send_msg,&response);
        return err;
}
...
int stm_lpm_get_wakeup_device(enum stm_lpm_wakeup_devices *wakeup_device){
        ...
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_GET_WUD,NULL,MSG_ZERO_SIZE,MSG_ID_AUTO,SBC_REPLY_NO_IRQ);
        err=lpm_exchange_msg(&send_msg,&response);
...
int stm_lpm_setup_fp(struct stm_lpm_fp_setting *fp_setting){
        ...
        msg[0]= ((fp_setting->owner&OWNER_MASK)|((fp_setting->am_pm&1)<<2)|(0<<3)|((fp_setting->brightness&BRIGHT_MASK)<<4));
        LPM_FILL_MSG(send_msg,STM_LPM_MSG_SET_FP,msg,1,MSG_ID_AUTO,SBC_REPLY_YES);
        err=lpm_exchange_msg(&send_msg,&response);
        return err;
}
