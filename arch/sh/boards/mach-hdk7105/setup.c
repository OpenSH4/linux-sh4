...
/*****YWDRIVER_MODI 2011-11-14 D26LF Add:
...
//#define HDK7105_PIO_PCI_SERR  stm_gpio(15, 4)//YWDRIVER_MODI lwj remove
#if 0 //YWDRIVER_MODI lwj modify
#define HDK7105_PIO_PHY_RESET stm_gpio(15, 5) //this is for demo
#else
#define HDK7105_PIO_PHY_RESET stm_gpio(5, 7)
#endif
//#define HDK7105_PIO_PCI_RESET stm_gpio(15, 7)//YWDRIVER_MODI lwj remove
...
/* Serial Flash */
/*****YWDRIVER_MODI 2011-11-14 D26LF Modi:
    Description:spi flash
*/
...
static struct spi_board_info hdk7105_serial_flash = {
	...
	.platform_data  = &(struct flash_platform_data) {
		.name = "m25p80",
		//.type = "m25p32",		//为了识别多种spi flash所以不指定型号
		.nr_parts	= 1,
		.parts = (struct mtd_partition []) {
			{
				.name = "uboot",
...
/*	&hdk7105_leds,*/
	&hdk7105_front_panel,
	//&hdk7105_nor_flash,		//YWDRIVER_MODI lwj remove
};

/*****     2012-03-02     *****/
//YWDRIVER_MODI add by lf for for spi flash start
...
//YWDRIVER_MODI add by zhp for usb overflow start
static struct task_struct *kusb_oc_check;
static int usb_oc_thread(void *__unused)
{
	int usb0_io = 0;
	int usb1_io = 0;
	int usb0_io_old = -1;
	...
	while(true)
	{
		usb0_io = gpio_get_value(stm_gpio(4, 6));
		if(usb0_io_old != usb0_io)
		{
			usb0_io_old = usb0_io;
			gpio_set_value(stm_gpio(4, 7),usb0_io);
			//printk(KERN_INFO "set usb0 %d\n", usb0_io);
			msleep(1);
			if(usb0_io == 0)
			{
				msleep(500);
		...
		usb1_io = gpio_get_value(stm_gpio(4, 4));
		if(usb1_io_old != usb1_io)
		{
			usb1_io_old = usb1_io;
			gpio_set_value(stm_gpio(4, 5),usb1_io);
			//printk(KERN_INFO "set usb1 %d\n", usb1_io);
			msleep(1);
			if(usb1_io == 0)
			{
				msleep(500);
...
//YWDRIVER_MODI add by zhp for usb overflow end
...
/***** 2011-11-14 D26LF Modi end ****/
	...
	#if 0 //YWDRIVER_MODI modify by zhp for usb start
	...
	stx7105_configure_usb(1, &(struct stx7105_usb_config) {
			...
			.routing.usb1.pwr = stx7105_usb0_pwr_pio4_5, });
	#endif	//YWDRIVER_MODI modify by zhp for usb end

	//YWDRIVER_MODI add by zhp for usb overflow start
	gpio_request(stm_gpio(4, 7), "Usb0 en");
	gpio_direction_output(stm_gpio(4, 7), 1);
	gpio_request(stm_gpio(4, 5), "Usb1 en");
	gpio_direction_output(stm_gpio(4, 5), 1);
	gpio_set_value(stm_gpio(4, 5),1);
	gpio_set_value(stm_gpio(4, 7),1);
	kusb_oc_check = kthread_run(usb_oc_thread, NULL, "kusb_oc_check");
	//YWDRIVER_MODI add by zhp for usb overflow end
